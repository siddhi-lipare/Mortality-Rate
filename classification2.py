...
class Classification:
    ...
    def classification_pipeline(self):
        if self.clf_opt == 'rf':
            ...
            param_grid = {
                'clf__penalty': ['l1', 'l2'], # Norm used in the penalization
                'clf__C': [0.1, 0.01, 0.5, 0.02], # Regularization parameter
            ...
            be1 = RandomForestClassifier(n_estimators=100, random_state=42, min_samples_split=2, criterion='gini') # After hyperparameter tuning of RF these are the results
            be2 = LogisticRegression(solver='saga', class_weight='balanced', penalty='l1', random_state=42, tol=0.0001, max_iter=70) # After hyperparameter tuning of LR these are the results
            be3 = DecisionTreeClassifier(max_depth=10, criterion='entropy', max_features='sqrt', min_samples_split=6, ccp_alpha=0.01) # After hyperparameter tuning of DT these are the results
            clf = AdaBoostClassifier(algorithm='SAMME.R', n_estimators=100) # After hyperparameter tuning of AdaBoost these are the results
            param_grid = {
                'clf__base_estimator': [be1, be2, be3], # Base estimator
                'clf__learning_rate': [0.01, 0.1, 1, 0.02, 0.03], # Learning rate shrinks the contribution of each classifier by learning_rate
    ...
    def get_imputer(self):
        if self.impute_opt == 'mean': 
            ...
            return KNNImputer(n_neighbors=4, weights='uniform', metric='nan_euclidean') # Impute missing values using KNN imputation method
        else:
    ...
    def classification(self):
        ...
        categorical_columns = ['gendera', 'hypertensive', 'atrialfibrillation', 'CHD with no MI', 'diabetes',
                                'deficiencyanemias', 'depression', 'Hyperlipemia', 'Renal failure', 'COPD']

        # Convert to categorical data type
        X[categorical_columns] = X[categorical_columns].astype('category')
        X_testdata[categorical_columns] = X_testdata[categorical_columns].astype('category')
        ...
        X_testdata = pd.concat([X_testdata.drop(categorical_columns, axis=1), X_testdata_encoded], axis=1)
        ...
        # feature_selection = SelectKBest(score_func=mutual_info_classif, k=self.no_of_selected_features)
        # pipeline_steps = [('imputer', imputer), ('feature_selection', feature_selection), ('clf', clf)]
        # pipeline = Pipeline(pipeline_steps)
